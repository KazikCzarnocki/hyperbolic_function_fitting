---
title: "Estimation of theta and delta"
author: "Kazik"
date: "02.06.2021"
...
saemix.options <- saemixControl(map=TRUE, fim=TRUE, ll.is=TRUE, displayProgress=FALSE, seed=632545, print=FALSE, save = FALSE)
...
$ip=\frac{\theta *75}{1+\delta *SD}$

$\frac{75}{ip}=\frac{1}{\theta}+\frac{\delta}{\theta}*SD$

Let, $y=\frac{1}{ip}$ and $x=\frac{SD}{75}$ and we obtain a linearized form.

```{r, eval=FALSE}
#select data for the first respondent
a <- filter(data_long, id==1)
#optymize using coeff from the linearized equation as a staring point
a$y <- 75/a$ip
a$x <- a$sd
b <- lm(y~x, a)
theta_start <- as.numeric(1/b$coefficients[1])
delta_start <- as.numeric(b$coefficients[2]*theta_start)


nls(ip ~ (theta*75)/(1+delta*sd), data=a, start = c(theta=theta_start, delta=delta_start))
...
Let's try something slightly better.
